...
for (i in 1:itin_max_1) {
  ...
  result_1[[i]] <- get_itinerary_times(itin, subway_data.2) %>% mutate(itin_id= i, group= 'red')
}

plot_distribution_for_itinerary(result_1[[1]])
...
for (i in 1:itin_max_2) {
  ...
  result_2[[i]] <- get_itinerary_times(itin, subway_data.2) %>% mutate(itin_id=i, group= 'orange')
}
...
all_itin_df <- compute_all_itins(test_train, subway_data.2)
...
mj_data <- read_csv('../../data/itineraries-Midtown-Jamaica.csv') %>%
  read_csv('../../data/itineraries-1-train.csv') %>% 
  filter(event != "travel", itinerary_id < 2) %>%
  mutate(stop_station = paste(station, direction, sep = ''))

all_itin_df <- compute_all_itins(mj_data, subway_data.2)
plot_densities(all_itin_df)
...
for (i in 1:itin_max) {
  ...
  result[[i]] <- get_itinerary_times(itin, subway_data.2) %>% mutate(itin_id=sprintf("%s: %s", i, label))
}
