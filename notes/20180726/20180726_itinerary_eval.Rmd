...
for (i in 1:itin_max_1) {
  ...
  result_1[[i]] <- get_itinerary_times(itin, subway_data) %>% mutate(itin_id= i, group= 'red')
}
...
for (i in 1:itin_max_2) {
  ...
  result_2[[i]] <- get_itinerary_times(itin, subway_data) %>% mutate(itin_id=i, group= 'orange')
}
...
all_itin_df <- compute_all_itins(test_train, subway_data)
...
mj_data <- # read_csv('../../data/itineraries-Midtown-Jamaica.csv') %>%
  ...
  filter(event != "travel", itinerary_id < 5)
