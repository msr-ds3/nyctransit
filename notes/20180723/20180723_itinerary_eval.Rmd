...
get_itinerary_times <- function(itinerary, start_time) {
  ...
  transfer_time <- ifelse(nrow(data.frame()) == 0, 0, as.integer(transfer_special_case$weight))
  ...
  adjusted_start <- as.POSIXct(start_time, origin = "1970-01-01", tz = "UTC") +
    as.integer(transfer_special_case$weight)
  print(adjusted_start)
  
  curr_itin <- itinerary %>% anti_join(transfer_special_case)
...
test_itin <- test_train %>% filter(itinerary_id == 1)
