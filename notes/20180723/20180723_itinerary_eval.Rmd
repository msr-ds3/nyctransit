...
result[[i]] <- get_itinerary_times(itin) 
print(result[[i]])
}


get_itinerary_times <- function(itinerary) {
  trains <- itinerary %>% .$line %>% unique()
  stops_data <- itinerary %>% select(leg, stop_station) %>% split(.$leg)
  stop_station <- list()
  ...
  output_df$time_diff <- output_df[, ncol(output_df)] - output_df[, 3]
