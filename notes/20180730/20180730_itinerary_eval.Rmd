...
line_color <- routes %>% group_by(route_color) %>% select(route_short_name, route_color) %>% arrange(route_color)
hex_color <- data.frame(route_color= c(unique(line_color$route_color)),
                          color_name= c("black","dark green","green", "blue","light green", "dark gray", "brown", "light gray", "purple", "red",  "yellow"  ,"orange" ))
...
Looking at trip time distributions between 86 St and 23 St on the Red Line (1, 2, 3) versus the Orange Line (B, D, F, M) 
...
plot_densities(mj_df, bins = 30)
plot_densities(test_mj_df, bins = 30)

# TODO: look through all trips to see if they have the same number of stops
#   - if they do, then this may mean the MTA is holding the trains
...
plot_densities(mj_bm_comparison_df)
```
...
# plot distributions
plot_densities(af_complex_itins)
plot <- plot_densities(af_complex_itins, bins = 60)
plot


ggsave(plot = plot, file = here("figures", "distribution-of-wait-times-from-atlantic-to-fulton-super-network.png"), width = 4, height = 4)
...
# plot distributions
plot_densities(aus_complex_itins)
```
...
# plot distributions
plot_densities(abp_complex_itins)
```
...
# plot distributions
plot_densities(agc_complex_itins)
```
...
# plot distributions
plot_densities(ats_complex_itins)
```
