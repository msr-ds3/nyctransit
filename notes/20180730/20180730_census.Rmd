...
load('../../data/wait_times_filtered_df.RData')
...
plot_50 <- ggplot(filtered_grid, aes(x = median_income, y = time_50P/60 + walking_mins, color = borough)) +
  geom_point(alpha = 0.2) +
  xlab('Median Household Income') +
  scale_x_continuous(label = comma) +
  ylab('50th Perc. Trip Time to 34th St') +
...
multiplot(plot_50, plot_90)
# ggsave("../../figures/income_v_50P_time_to_34.png", plot = plot_50, height = 7, width = 10)
...
bronx <- filtered_grid %>% filter(borough == "Bronx", median_income >= 75000) 
bronx_income <- leaflet(nyc_neighborhoods) %>%
      addTiles() %>%
      addProviderTiles("CartoDB.Positron") %>%
      setView(-73.87, 40.83, zoom = 11) %>%
  addCircleMarkers(data = bronx, lat = ~lat, lng = ~lon, radius = 6, stroke = FALSE, color = "red")

mapshot(bronx_income, url = "bronx_income_75000.html", 
                                       file ="bronx_income_75000.png")
```
