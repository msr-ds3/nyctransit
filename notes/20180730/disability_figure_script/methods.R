create_grid <- function(lat, lon, neighborhood, borough_blacklist = c(),  by = 0.002){
  ...
  grid %>% filter(!is.na(neighborhood) & !borough %in% borough_blacklist)
}
...
compute_point2station <- function(point2station_matrix,speed, k = 10, stop_whitelist = NULL){
  ...
  mutate(travel_time = speed * distance, 
         subway_time = distance_matrix[stop_id, dest]/60,
...
create_map <- function(point2station_data, pal, color_col = 'total_time', dest, stops, position = "bottomright", title = "Commute time to", bins = 7){
  
  stop <- stops[stops$stop_id == dest,]
  stop_lon <- stop[[3]]
  stop_lat <- stop[[2]]
  stop_name <- stop[[4]]
  pal_col <- pull(point2station_data, color_col)

  leaflet(nyc_neighborhoods) %>%
    addTiles() %>%
    addProviderTiles("CartoDB.Positron") %>%
    setView(-73.98, 40.75, zoom = 11.25, options = c('zoomSnap:.25')) %>%
