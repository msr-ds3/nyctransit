...
vertices <- vertices[vertices != "R19S"]
station_list <- list()
trip_time <- list()
for(i in 1:length(vertices)){
  path <- get_itinerary_directed(mta_igraph, vertices[i], to, 1, stops = stops,
                                 attributeNames = '90%')
...
filtered_grid <- grid %>% 
  ...
  mutate(min_interval = cut(total_time, seq(0, max(total_time), by = 5), include.lowest = TRUE),
         bin = cut(total_time, seq(0, max(total_time), by = 5), include.lowest = TRUE, labels = FALSE))
```
