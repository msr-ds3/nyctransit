...
```{r red-line-data}
red_line_data <- read_csv(here("data", "itineraries-Red-Line-96-23-train.csv"))
...
```{r orange-line-data}
orange_line_data <- read_csv(here("data", "itineraries-Orange-Line-96-23-train.csv"))
...
```
### Midtown to Mets Willets (LGA)

```{r Bryant Park- Mets Willets}
bm_data <- read_csv(here("data","itineraries-5ave-MetsWillets.csv")) %>%
  filter(event != "travel", itinerary_id < 2)

all_itin_df.2 <- compute_all_itins(bm_data, subway_data) %>% filter(time_diff > 2) # filtering out 0's from E train
plot_densities(all_itin_df.2)
...
red_line_data <- read_csv(here("data", "itineraries-96-Fulton-red.csv"))
...
green_line_data <- read_csv(here("data", "itineraries-96-Fulton-green.csv"))
