...
load(file = here::here("data", "todd_subway_realtime.RData"))
source(file = here::here("src", "itinerary_evaluation.R"))
```
...
debug_itin_plot <- function(itin_df, subway_data, start_hour = 8, end_hour = 9) {
  filtered <- itin_df %>% 
    filter(hour(leg1_start_time) >= start_hour & hour(leg1_start_time <= end_hour))
  
  filtered %>% mutate(itin = row_number()) %>%
    filter(itin <= 5) %>%
    ggplot() +
    geom_segment()
