...
idw <- idw(formula=time_diff ~ 1, locations=filtered_grid_spdf, newdata = filtered_grid_spdf)  # apply idw model for the data
...
grid_pal <- colorNumeric(
  palette = clrs,
  domain = filtered_grid$time_diff,
  reverse = TRUE,
...
m <- leaflet(nyc_neighborhoods) %>% 
  ...
  addLegend("bottomright", pal = grid_pal, values = filtered_grid$time_diff,
    title = "Commute time to 23 St",
