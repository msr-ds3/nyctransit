...
df <- exp_wait_time_and_weather %>% 
  ...
  distinct(stop_mta_id, route_mta_id, time_of_day, day, avg_precip, pred_median_wait_precip_I, pred_90th_wait_precip_I) %>% 
...
pal <- colorNumeric(
  palette = clrs,
  domain = log(df$pred_median_wait_precip_I),
  reverse = TRUE)
...
map_diff <- 
  ...
  addCircleMarkers(~long, ~lat,
                   popup = paste("<b>", df$stop_name, "</b>",
                                 ...
                                 "<br/>", round(df$pred_median_wait_precip_I/60, 2), "minutes",
                                 "<br/>", substring(df$stop_mta_id, 4,4), "Bound Train"),
                   radius = 5,
                   color = ~pal(log(pred_median_wait_precip_I)),
                   stroke = FALSE, 
                   fillOpacity = 0.5) %>%
  addLegend("bottomright", pal = pal,
              title = "Additional Commute Time", value = round(df$pred_median_wait_precip_I/60, 2),
...
leaflet(df2) %>%
  ...
  addCircleMarkers(~long, ~lat,
                   ...
                   fillOpacity = 0.5) %>%
  addLegend("bottomright", pal = pal,
              title = "Additional Commute Time", value = round(df2$diff/60, 2),
