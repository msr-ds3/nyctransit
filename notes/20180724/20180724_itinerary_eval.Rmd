...
test_data <- read_csv('../../data/itineraries_1_train.csv') 
...
```{r Frontier plot}
#Frontier plot
frontier_df <- bind_rows(result)

plot_data <- frontier_df %>% group_by(itin_id) %>% mutate(avg= mean(time_diff), stdv= sd(time_diff))
...
# or pull out the number from the ROCR package with pred <- predictions(predicted, actual); performance(pred, 'auc')@y.values[[1]]
