...
filtered %>%
  ggplot() +
  geom_histogram(aes(x = time_diff, group=itin_id, col=itin_id, fill=itin_id))
...
plot_distribution_for_itinerary(plot_data)

# NOTE: DEBUG CODE
# start.time <- Sys.time()
# test_df <- get_itinerary_times(test_itin)
# end.time <- Sys.time()
...
pred <- predictions(predicted, actual); performance(pred, 'auc')@y.values[[1]]
