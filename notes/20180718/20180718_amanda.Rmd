...
```{r}
#long_wait <- exp_wait_time_and_weather %>% 
#  group_by(stop_mta_id, hour, day) %>% 
#  mutate(long_wait = ifelse(exp_wait > median(exp_wait), 1, 0)) %>% 
#  left_join(weather_2018)

#set.seed(41815)
#sample <- sample.int(n = nrow(long_wait), 
#                     size = floor(.05*nrow(long_wait)), 
#                     replace = F)
#train <- long_wait[sample, ]
#test  <- long_wait[sample, ]
#logit <- glm(long_wait ~ stop_mta_id + PRCP + route_mta_id + day + hour, data = train, family = "binomial")
#summary(logit)
#test$y_hat <- predict(logit, test, type = "response")
