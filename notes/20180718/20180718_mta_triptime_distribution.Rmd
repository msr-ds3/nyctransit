...
# <- data.frame("itin_id" = c("1","1","1"), "station"= stop_station[1], stop_ )
...
for (i in length(trains)) {
  ...
  # TODO: filter subway_data to get appropriate information for each train
   leg_data <- subway_data.2 %>%
    ...
    arrange(arrival_time)
  # append to list
  train_data <- list(train_data, list(leg_data))
}
...
local_with_express_diffs$group <- "local w/ express"
