...
realtime <- read_csv('../../data/todd_subway_realtime.csv', col_types = cols(route_mta_id = col_factor(levels = NULL)))
```
...
igraph_edges <- full_sequences %>% filter(!is.na(prev_stop_id)) %>% select(prev_stop_id, stop_id, weight)
...
shortest_path <- shortest_paths(mta_igraph, from = c("701"), to = c("R19"), mode="in")
