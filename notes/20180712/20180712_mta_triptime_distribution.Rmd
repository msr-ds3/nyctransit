...
plot_local_express <- function(local_line, express_line, direction, start_stop_id, end_stop_id)
{
  direction_num <- ifelse(direction == "N", 1, 3)
  #local train
  local_train <- subway_data %>%
  filter(stop_mta_id == sprintf("%s%s", start_stop_id, direction) | stop_mta_id == sprintf("%s%s", end_stop_id, direction),
         route_mta_id == local_line) %>%
  mutate(day_of_week = weekdays(as.Date(departure_time)),
...
plot_local_express ("1","2", "S", "120S", "132S")
```
