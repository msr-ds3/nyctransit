...
get_itinerary_times <- function(itinerary) {
  ...
  station_label <- curr_itin %>% .$line %>% rle %>% .$values %>% paste(collapse = " => ")
  trains_label <- curr_itin %>% .$stop_name %>% rle %>% .$values %>% paste(collapse = " => ")
  label <- sprintf("%s @ %s", station_label, trains_label)
...
test_data_org <- read_csv('../../data/itineraries-Orange-Line-96-23-train.csv')
...
plot_data %>% select(itin_id, avg, stdv) %>% distinct()
...
# TODO: Refactor auc_function to require less stuff up_front
...
filtered %>%
  ...
  theme(legend.position = "bottom", legend.direction = "vertical", legend.key = element_rect(size = 5), legend.key.size = unit(1.5, 'lines'))
  # geom_histogram(), position = "identity", alpha = 0.5)
