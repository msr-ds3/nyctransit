...
while (i <= N) {
  ...
  end_time <- train_data[[1]]$arrival_time[i]
  ...
  for (ii in 2:length(trains)) {
    while ((counters[ii] <= nrow(train_data[[ii]])) &
           (train_data[[ii]]$departure_time[counters[ii]] < end_time)) {
    ...
    end_time <- train_data[[ii]]$arrival_time[curr_idx]
    row_info <- c(row_info,
...
plot_data %>% arrange(start_time) %>% mutate(itin = row_number()) %>% ggplot(aes(color = route_mta_id)) + geom_segment(aes(x = start_time, xend = departure_time, y = itin, yend = itin)) + geom_text(aes(label = realtime_trip_id, x = start_time, y = itin))
