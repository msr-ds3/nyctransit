...
```{r}
line_num_trips <- all_trips %>% group_by(route_id, trip_id) %>%  filter(hour(trip_start_time) %in% time_filter) %>%
  filter(day_of_week %in% day_filter) %>% distinct() %>% group_by(route_id) %>% summarize(num_trips = n())
...
unique_sequences <- all_trips %>%
  ...
  select(trip_id, route_id, direction_id, stop_id, stop_name, prev_stop_id, prev_stop_name) %>%
  group_by(route_id, direction_id, stop_id, stop_name, prev_stop_id, prev_stop_name) %>%
  summarize(count = n()) %>% left_join(line_num_trips) %>% 
  mutate(freq = count/num_trips * 100) %>% ungroup %>% filter(freq > (quantile(freq, .10, names = FALSE)[[1]])) %>%
  select(-freq)
```
...
####Stops
```{r}
stop_attributes <- stops %>% select(stop_id, stop_name, stop_lat, stop_lon)
```
...
itin <- itin %>% left_join(stop_attributes, by = c("station" = "stop_id")) %>% select(itinerary_id, station, stop_name, line, direction, event_id, event, leg)
```
...
itin_map_data <- itin %>% left_join(stop_attributes, by = c("station" = "stop_id", "stop_name" = "stop_name"))
itin_map_data <- itin_map_data %>% extract(line, c("route_id"),"([^_][^_]?)_?.*", remove = FALSE) %>% left_join(routes)
