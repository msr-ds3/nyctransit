...
ssmd <- function(graph, from, to = V(graph)){
  get.all.shortest.paths(graph, from, to = to, mode = 'out')$res %>%
    lapply(compute_path_tibble, graph = graph) %>%
    reduce(rbind)
}

ssmd_walking <- function(graph, from_lat, from_lon){

}

ssmd_itinerary <- function(graph, from){
  paths <- get.all.shortest.paths(mta_igraph, from, mode = 'out')$res %>% lapply(names)
  path_tables <- paths_to_tibble(graph = mta_igraph, paths)

  #path_tables %>% greedy(num_itineraries = length(paths))
  path_tables
}
